...
\]
be the set of all finite token sequences over $\mathcal{V}$. An element $T \in \mathcal{T}$ is called a \emph{text sample}. Its token length is denoted by $|T|$.
...
\subsection{Theorem-level statements versus modelling assumptions}
